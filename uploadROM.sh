...
# Setup Rclone Config qua API (Tương thích với bước Setup trong file YML)
if [ "$1" == "setup" ]; then
  if [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ]; then
...
if [[ $rom_os == "MIUI" ]] || [[ "$base_rom_code" != OS* ]]; then
    os_type="MIUI"
    true_os="MIUI"
    uploaddir="MIUI"
else
...
# Xóa các file firmware không mong muốn (user requested)
...
# Tạo thư mục và sao chép an toàn (Khắc phục lỗi mất file images)
...
cp -rf $work_dir/bin/script2flash/META-INF $work_dir/out/${os_type}_${device_code}_${base_rom_code}/ 2>/dev/null || true
...
cp -rf $work_dir/bin/script2flash/*.install $work_dir/out/${os_type}_${device_code}_${base_rom_code}/ 2>/dev/null || true
...
share_link=$(rclone --config="$RCLONE_CONFIG_1DRIVE" link "$remote_file_path")
mkdir -p bin/ddevice
